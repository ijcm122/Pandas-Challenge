...
```python
# Table to summarizes metrics about every school
# School name
...
# Dataframe that holds results of above
...
#Display and arrange top five schools in passing rate
school_summary_df = school_summary_df.sort_values(["% Overall Passing Rate"], ascending=False)
#Optional: Formatting Data cleaner
...
```python
#Build a table listing the average Math Score for students of grade levels (9th, 10th, 11th, 12th) at each school.
#Data Frame for pandas series for each grade. 
...
#Build the series into a dataframe
